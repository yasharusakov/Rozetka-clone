import search from '../../assets/resources/images/search.png'
    ...
    return (
        <header className="header wrapper__header">
            <div className="header__container">
                <div onClick={() => setPopup({name: 'burgerPanel', type: true})} className="header__burger">
                ...
                {
                    !userState && (
                        <div onClick={() => setPopup({name: 'login', type: true})} className="header__user">
                            <img src={user} alt="user"/>
                        </div>
                    )
                }
                <div style={userState ? {marginLeft: 0} : {}}
                     onClick={() => setPopup({name: 'basket', type: true})}
                     className="header__shopping-basket">
                    <img src={basket} alt="basket"/>
                    {inCart.length && <div className="header__shopping-basket__counter">{inCart.length}</div>}
                </div>
            </div>
            <BurgerPanel/>
